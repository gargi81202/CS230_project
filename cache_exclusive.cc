...
{
  int set = get_set(full_addr);
    for (int j = 0; j < NUM_WAY; j++)
    {
      if (block[set][j].full_addr == full_addr) 
      {
        block[set][j].valid = false;
        return;
      }
    }
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        PACKET add_to_pq;
        add_to_pq.full_addr = block[set][way].full_addr;
        add_to_pq.data = block[set][way].data;
        add_to_pq.ip = block[set][way].ip;
        add_to_pq.cpu = block[set][way].cpu;
        add_to_pq.instr_id = block[set][way].instr_id;
        add_to_pq.address = block[set][way].address;
        add_to_pq.delta = block[set][way].delta;
        add_to_pq.type = PREFETCH;
        add_to_pq.confidence = block[set][way].confidence;
        add_to_pq.depth = block[set][way].depth;
        add_to_pq.signature = block[set][way].signature;
        add_to_pq.event_cycle = block[set][way].cpu;
        add_to_pq.fill_level = fill_level << 1;

        if (lower_level && add_to_pq.address > 0 && add_to_pq.rob_index > 0) lower_level->add_pq(&add_to_pq);

        if (lower_level) lower_level->remove_line(MSHR.entry[mshr_index].full_addr);
        ...
        if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
            ...
            // lower_level->update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0);
            // COLLECT STATS
	  ...
	  if (cache_type == IS_L1I)
		...
		cpu = 0;
            ...
            sim_miss[fill_cpu][MSHR.entry[mshr_index].type]++;
            sim_access[fill_cpu][MSHR.entry[mshr_index].type]++;
            // if (lower_level) lower_level->add_pq(&MSHR.entry[mshr_index]);
            // if (lower_level) lower_level
		...
		if(mshr_index == -2)
		  ...
		  if (cache_type == IS_L1I)
			...
			cpu = 0;
                    ...
                    sim_access[writeback_cpu][WQ.entry[index].type]++;
                    if (lower_level) lower_level->remove_line(WQ.entry[index].full_addr);
                    fill_cache(set, way, &WQ.entry[index]);

                    PACKET * add_to_pq = new PACKET();
                    add_to_pq->full_addr = block[set][way].full_addr;
                    add_to_pq->data = block[set][way].data;
                    add_to_pq->ip = block[set][way].ip;
                    add_to_pq->cpu = block[set][way].cpu;
                    ...
                    if (lower_level) lower_level->add_pq(add_to_pq);
